...
class LorawanSender : public rclcpp::Node
{
public:
    LorawanSender() : Node("lorawan_send")
    {
        ...
        subscription_ = this->create_subscription<nav_msgs::msg::Odometry>(
            ...
            std::bind(&LorawanSender::odom_callback, this, std::placeholders::_1));
...
private:
    ...
    void write_serial(const char *serial_cmd)
    {
        ...
        {
            RCLCPP_ERROR(this->get_logger(), "No response from LoRa module");
        }
    }

    void lorawan_setup(){
        write_serial("AT+MODE=LWOTAA");
        write_serial("AT+DR=AS923");
        choose_dr();
        write_serial("AT+ADR= OFF");
        write_serial("AT+CLASS=C");
        write_serial("AT+KEY=APPKEY,\"c387364ecd4176e78fe849a0b8c70580\"");//app key: c387364ecd4176e78fe849a0b8c70580
        RCLCPP_INFO(this->get_logger(),"LoRaWAN settings setup done");

        write_serial("AT+JOIN");
    ...
    void choose_dr(){
        ...
        {
        case "7":
            write_serial("AT+DR=5");
            break;
        case "8":
            write_serial("AT+DR=4");
            break;
        case "9":
            write_serial("AT+DR=3");
            break;
        case "10":
            write_serial("AT+DR=2");
            break;
        case "11":
            write_serial("AT+DR=1");
            break;
        case "12":
            write_serial("AT+DR=0");
            break;
        default:
            RCLCPP_ERROR("SF not defined, default to SF7 / DR=5");
            write_serial("AT+DR=5");
            break;
        }
    }
